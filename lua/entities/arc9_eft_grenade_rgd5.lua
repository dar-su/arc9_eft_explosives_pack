...
local dmg = 200
local dmgradiusminM = 3 * 2
local dmgradiusmaxM = 7 * 1.5
...
function ENT:Detonate()
    ...
    if self:WaterLevel() > 0 then
        ...
        ParticleEffect("grenade_final", selfpos, Angle(0, 0, 0), nil)
        ...
        if checkindoor(self) then
            self:EmitSound(path .. "gren_expl1_indoor_close.ogg", 140, 100, 10, nil, nil, 0)
            self:EmitSound(path .. "gren_expl1_indoor_close.ogg", 140, 100, 10, nil, nil, 0) -- Louder!!!!!
            self:EmitSound(path .. "gren_expl1_indoor_close.ogg", 140, 100, 10, nil, nil, 0) -- Louder!!!!!

            self:EmitSound(path .. "gren_expl1_indoor_distant.ogg", 0, 100, 0.5, nil, nil, 0)

        else
            self:EmitSound(path .. "gren_expl1_close.ogg", 140, 100, 10, nil, nil, 0)
            self:EmitSound(path .. "gren_expl1_close.ogg", 140, 100, 10, nil, nil, 0) -- Louder!!!!!
            self:EmitSound(path .. "gren_expl1_close.ogg", 140, 100, 10, nil, nil, 0) -- Louder!!!!!
            
            -- self:EmitSound(path .. "gren_expl1_distant.ogg", 250, nil, nil, nil, nil, 0)
            self:EmitSound(path .. "gren_expl1_far.ogg", 0, 100, 0.5, nil, nil, 0)
        end 
        ...
        for i = 1, #potentionalcontusioned do
            if potentionalcontusioned[i]:IsPlayer() then
                ...
                local contmult = (shakeradiusM - selfpos:Distance(ply:GetPos()) * 0.0254) / shakeradiusM -- how close we are

                net.Start("arc9eftexplosion")
                net.WriteFloat(contmult)
                net.WriteUInt(contusionLength, 9)
                net.WriteBool(false) -- not flashbanga
                net.Send(ply)

                ply:ViewPunch(Angle(2, 0, -10) * contmult)
            end
        end
    end
